...
-(instancetype)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier{
    ...
    if (self) {
        self.effect = [self setLabelWith:17*W_Scale color:RGB(51, 51, 51)];
        self.time = [self setLabelWith:13*W_Scale color:RGB(102, 102, 102)];
        self.amount =[self setLabelWith:17*W_Scale color:RGB(51, 51, 51)];
        ...
        [self.effect mas_makeConstraints:^(MASConstraintMaker *make) {
            make.left.mas_equalTo(36*W_Scale);
            make.top.mas_equalTo(15*W_Scale);
        }];
        [self.time mas_makeConstraints:^(MASConstraintMaker *make) {
            make.left.mas_equalTo(36*W_Scale);
            make.top.mas_equalTo(weakSelf.effect.mas_bottom).offset(15*W_Scale);
        }];
        
        [self.amount mas_makeConstraints:^(MASConstraintMaker *make) {
            make.centerY.mas_equalTo(weakSelf.contentView);
            make.right.mas_equalTo(-35*W_Scale);
        }];
    }
    return self;
}
