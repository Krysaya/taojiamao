...
#pragma mark bb改 -之前返回的是id  我直接进行解析成字典回传
- (NSDictionary*)getCacheResponseObjectWithRequestUrl:(NSString *)requestUrl
                                    params:(NSDictionary *)params {
    assert(requestUrl);
    ...
    if (cacheData) {
        if ([cacheData isKindOfClass:[NSDictionary class]]) {
            ...
            NSDictionary * dict  = [NSJSONSerialization JSONObjectWithData:cacheData options:0 error:&error];
            return dict;
        }
    }else{
        return nil;
