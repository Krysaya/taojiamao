    ...
    [manager setReachabilityStatusChangeBlock:^(AFNetworkReachabilityStatus status) {
        ...
        switch (status) {
            case AFNetworkReachabilityStatusNotReachable:
                ...
                DSLog(@"没有网络");
                [[NSNotificationCenter defaultCenter]postNotificationName:TJNotifyNetworkingNON object:nil];
...
+ (XDURLSessionTask *)getWithUrl:(NSString *)url
    ...
    //添加固定参数
    params = [self addFixationParameter:params];
    ...
    session = [manager POST:url
                 parameters:params
                   progress:^(NSProgress * _Nonnull uploadProgress) {
                       ...
                       //增加默认为200 其他提示用户
//                       NSNumber * err_code = responseObject[@"err_code"];
//                       if([err_code intValue]==200){
                           if (successBlock) successBlock(responseObject);
...
#pragma mark - put
+ (XDURLSessionTask *)putWithUrl:(NSString *)url
                  refreshRequest:(BOOL)refresh
                           cache:(BOOL)cache
                          params:(NSDictionary *)params
                    successBlock:(XDResponseSuccessBlock)successBlock
...
#pragma mark - delete
+ (XDURLSessionTask *)deleteWithUrl:(NSString *)url
                     refreshRequest:(BOOL)refresh
                              cache:(BOOL)cache
                             params:(NSDictionary *)params
                       successBlock:(XDResponseSuccessBlock)successBlock
...
#pragma mark - patch
+ (XDURLSessionTask *)patchWithUrl:(NSString *)url
                    refreshRequest:(BOOL)refresh
                             cache:(BOOL)cache
                            params:(NSDictionary *)params
                      successBlock:(XDResponseSuccessBlock)successBlock
...
#pragma mark -bb添加固定参数
+(NSDictionary*)addFixationParameter:(NSDictionary*)dict{
    
//    NSString * islogin = GetUserDefaults(HADLOGIN);
//    NSMutableDictionary * md = [NSMutableDictionary dictionaryWithDictionary:dict];
//    if (islogin!=nil && islogin.length>0) {
//        DSLog(@"登录了");
//    }else{
//        DSLog(@"未登录");
//        [md setValue:GetUserDefaults(Auth_code) forKey:Auth_code];
//        [md setValue:IOS forKey:ST];
//        [md setValue:CompanyID forKey:Auth_client_id];
//    }
//    return md.copy;
    return dict;
}
