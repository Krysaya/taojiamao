...
@interface TJPersonalController ()<UINavigationControllerDelegate,UITableViewDelegate,UITableViewDataSource,TJButtonDelegate>
...
// 芬兰
    self.midView = [[TJMiddleView alloc]init];
//    self.headTView.clipsToBounds = YES;
    self.midView.layer.cornerRadius = 8;
    self.midView.layer.masksToBounds = YES;
    self.midView.backgroundColor = [UIColor whiteColor];
    [self.headTView addSubview:self.midView];
    [self.midView mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.mas_equalTo(weakSelf.headIcon.mas_bottom).offset(18*W_Scale);
        make.centerX.mas_equalTo(weakSelf.headTView.mas_centerX);
        make.width.mas_equalTo(345*W_Scale);
        make.height.mas_equalTo(72*H_Scale);
